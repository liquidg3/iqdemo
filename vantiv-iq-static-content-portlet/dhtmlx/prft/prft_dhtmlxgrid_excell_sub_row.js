...
function eXcell_sub_row(cell){
	...
	this._setState = function(m,v){
		//the below is the modifed line - the orig has width and heigt of 18...
		(v||this.cell).innerHTML="<img src='"+this.grid.imgURL+m+"' width='13' height='13' />";
	...
	this.open = function (){
 	...
 	this.setValue = function(val){
 		if (val)this.grid.setUserData(this.cell.parentNode.idd,"__sub_row",val);
...
/*
The below is the untouched protoypes and function declarations....
they are merely here for convenience if they need to be overridden instead of having to copy
the orginal from dhtmlxgrid_excell_sub_row.js
...
function eXcell_sub_row_ajax(cell){
 	...
 	this.setValue = function(val){
 		if (val)this.grid.setUserData(this.cell.parentNode.idd,"__sub_row",val);
 		this.cell._sub_row_type="ajax";this._setState(val?"plus.gif":"blanc.gif");
 	}
};
...
function eXcell_sub_row_grid(cell){
 	...
 	this.getSubGrid = function(){
 		if (!cell._sub_grid)return null;return cell._sub_grid;
 	}
};
...
dhtmlXGridObject.prototype._expandMonolite=function(n,show,hide){
 	...
 	if (!that._sub_row_editor)that._sub_row_editor=new eXcell_sub_row(td);
 	if (!c)return;if (row._expanded && !show){
 		...
 		if (that._fake)that._fake.rowsAr[row.idd].style.height=(row.oldHeight||20)+"px";
 		...
 		if (td._previous_content){
 			...
 			var d=document.createElement("DIV");d.ctrl=td;
 			if (td._sub_row_type)that._sub_row_render[td._sub_row_type](that,d,td,c);
 			else d.innerHTML=c;
 		...
 		if (!that._flow){
 			...
 			that.attachEvent("onGroupStateChanged",function(){
 				this._correctMonolite();return true;
 			});
 			...
 			that.attachEvent("onEditCell",function(a,b,c){
 				if ((a!==2)&& this._flow[b] && this.cellType[c]!="ch" && this.cellType[c]!="ra") this._expandMonolite.apply(this._flow[b].ctrl.firstChild,[0,false,true]);
 				return true;
 			});
...
dhtmlXGridObject.prototype._sub_row_render={
	"ajax":function(that,d,td,c){
		...
		var xml=new dtmlXMLLoaderObject(function(){
			...
			if (z)for (var i=0;i<z.length;i++)eval(z[i].replace(/<([\/]{0,1})s[^>]*>/g,""));
			that._detectHeight(d,td)
 			that._correctMonolite();
 	...
 	"grid":function(that,d,td,c){
 		...
 		if (that.skin_name)td._sub_grid.setSkin(that.skin_name);
...
dhtmlXGridObject.prototype._correctMonolite=function(mode){
	...
	for (var a in this._flow)if (this._flow[a] && this._flow[a].tagName=="DIV")if (this.rowsAr[a]){
...
dhtmlXGridObject.prototype._collapsMonolite=function(){
	for (var a in this._flow)if (this._flow[a] && this._flow[a].tagName=="DIV")if (this.rowsAr[a])this.cells4(this._flow[a].ctrl).close();
};
